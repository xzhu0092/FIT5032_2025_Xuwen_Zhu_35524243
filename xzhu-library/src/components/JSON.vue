...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
       <li v-for="([type, count], index) in storeTypesEntries" :key="index">
       {{ type }}: {{ count }}
       </li>
      </ul>
      ...
      <ul>
        <li v-for="([day, hours], index) in openingHoursEntries" :key="index">
    ...
    <section class="lab-section">
      ...
      <ul>
       <li
        ...
        :style="author.name === 'George Orwell' ? { color: 'white', backgroundColor: '#42b883', fontWeight: 'bold' } : {}" 
...
const storeTypesEntries = computed(() => {
  return Object.entries(bookstores.storeTypes);
})
const openingHoursEntries = computed(() => {
